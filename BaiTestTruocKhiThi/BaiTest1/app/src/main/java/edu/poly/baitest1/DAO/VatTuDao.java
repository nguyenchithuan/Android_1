...
import edu.poly.baitest1.DbHelper.DbHelper;
...
public class VatTuDao {
    ...
    public VatTu selectOne(String tenVatTu) {
        VatTu vatTu = new VatTu();
        Cursor cursor = db.rawQuery("SELECT * FROM vatTu WHERE tenVatTu", new String[] {tenVatTu});
        if (cursor.moveToFirst()) {
            vatTu.setTenVatTu(cursor.getString(0));
            vatTu.setGiaTien(cursor.getDouble(1));
        }
